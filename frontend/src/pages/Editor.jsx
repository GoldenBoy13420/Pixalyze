...
export default function Editor() {
  const { activeTab, currentImage, processedImage, setActiveTab } = useStore()
  
  // If no image is uploaded, show the upload component
  if (!currentImage) {
    return (
  ...
  // Set to histogram if activeTab is upload (since we removed upload from sidebar)
  const effectiveTab = activeTab === 'upload' ? 'histogram' : activeTab
  const ActivePanel = panels[effectiveTab] || HistogramPanel
  const showResults = currentImage && effectiveTab !== 'upload'
